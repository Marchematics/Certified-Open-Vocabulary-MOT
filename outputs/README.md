...
- `milestones/materials_prospective_dft_followup/`: A3 prospective in-silico DFT follow-up protocol freeze. It fixes the ALIGNN-FF `alpha=0.10, K=500` design, 40/40/40 arm plan, public-label exclusion schema, novelty-crossmatch schema, DFT failure policy, and empty selection/job schemas. It is blocked until an unlabeled generated crystal pool and public crossmatch evidence are supplied.
