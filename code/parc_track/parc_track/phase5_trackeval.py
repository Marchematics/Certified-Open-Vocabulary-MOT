...
CONF_MATCHED_METHOD = "confidence_top_k_matched"
TRACK_EVAL_SCOPES = ("standard", "supported", "release_size")
...
def _track_key(ann: dict[str, Any], image_to_video: dict[int, int] | None = None) -> tuple[int, int]:
    video_id = int(ann.get("video_id", image_to_video.get(int(ann["image_id"]), -1) if image_to_video else -1))
    return video_id, int(ann.get("track_id", ann.get("id", 0)))


def _filter_annotations_for_scope(
    anns: list[dict[str, Any]],
    *,
    dataset: str,
    scope: str,
    selected_categories: set[int],
    release_size_k: int | None = None,
    image_to_video: dict[int, int] | None = None,
) -> list[dict[str, Any]]:
    """Filter GT annotations for paper-facing metric scopes.

    Standard scope is intentionally conservative: OVT-B keeps the full official
    GT denominator, while TAO keeps the existing supported-subset convention
    because TAO is federated. Supported/release_size scopes are explicitly
    paper-facing diagnostics and never replace the standard table.
    """
    if scope not in TRACK_EVAL_SCOPES:
        raise ValueError(f"unknown TrackEval scope: {scope}")
    filtered = list(anns)
    if selected_categories and (scope in {"supported", "release_size"} or dataset == "TAO"):
        filtered = [row for row in filtered if int(row.get("category_id", -1)) in selected_categories]
    if scope == "release_size":
        k = int(release_size_k or 0)
        if k <= 0:
            return []
        counts: dict[tuple[int, int], int] = {}
        for row in filtered:
            key = _track_key(row, image_to_video=image_to_video)
            counts[key] = counts.get(key, 0) + 1
        keep = {
            key
            for key, _ in sorted(counts.items(), key=lambda item: (-item[1], item[0][0], item[0][1]))[:k]
        }
        filtered = [row for row in filtered if _track_key(row, image_to_video=image_to_video) in keep]
    return filtered
...
def _selected_paths(
    ...
    if method in {CONF_METHOD, CONF_MATCHED_METHOD}:
...
def export_motchallenge_trackeval(
    ...
    scope: str = "standard",
    release_size_k: int | None = None,
    matched_to_method: str = PARC_METHOD,
) -> dict[str, Any]:
    ...
    parc_selected = _selected_paths(entry, cfg, universe, PARC_METHOD, alpha, seed, budget)
    matched_budget = int(len(parc_selected)) if release_size_k is None else int(release_size_k)
    for method in methods:
        if method == PARC_METHOD:
            selected = parc_selected.copy()
        elif method == CONF_MATCHED_METHOD:
            selected = _selected_paths(entry, cfg, universe, method, alpha, seed, matched_budget)
        else:
    ...
    if scope == "release_size" and release_size_k is None:
        release_size_k = int(selected_counts.get(matched_to_method, 0))
    benchmark = f"PARC_{dataset.replace('-', '')}_{generator}_a{_safe_alpha(alpha)}_s{seed}_m{budget}_{scope}"
    split = "train"
    ...
    for video_id, seq_name in seq_names_by_video.items():
        ...
        anns = _filter_annotations_for_scope(
            ann["anns_by_video"].get(video_id, []),
            dataset=dataset,
            scope=scope,
            selected_categories=selected_categories,
            release_size_k=release_size_k,
            image_to_video=None,
        )
    ...
    manifest = {
        ...
        "scope": scope,
        "release_size_k": int(release_size_k) if release_size_k is not None else None,
        "matched_to_method": matched_to_method,
        ...
        "scope_note": (
            "standard uses full OVT-B GT; TAO standard remains supported-subset because TAO annotations are federated."
            if scope == "standard"
            else "supported/release_size scopes are paper-facing diagnostics and do not replace standard HOTA."
        ),
...
def _summarize_trackeval_result(result: dict[str, Any], dataset_name: str, tracker: str) -> dict[str, Any]:
    ...
    return {
        "tracker": tracker,
        "method": CONF_MATCHED_METHOD
        if tracker.startswith(CONF_MATCHED_METHOD)
        else (CONF_METHOD if tracker.startswith(CONF_METHOD) else PARC_METHOD),
        "status": "success",
...
def run_trackeval_motchallenge(
    ...
    output_dir = ensure_data_output(out_dir or DATA_ROOT / "outputs/phase5_trackeval")
    manifest = export_motchallenge_trackeval(
        dataset,
        generator,
        alpha,
        seed,
        ...
        row.update(
            {
                ...
                "candidate_budget_M": budget,
                "scope": scope,
                "release_size_k": manifest.get("release_size_k"),
                "benchmark": manifest["benchmark"],
...
def run_trackeval_grid(
    ...
    out_csv = ensure_data_output(output_dir / f"table_trackeval_{dataset_slug}_{scope}.csv")
    merged.to_csv(out_csv, index=False)
    summary_csv = ensure_data_output(output_dir / f"table_trackeval_{dataset_slug}_{scope}_meanstd.csv")
    if not merged.empty:
        grouped = (
    ...
    result = {"status": "completed", "dataset": dataset, "scope": scope, "table": str(out_csv), "summary": str(summary_csv), "runs": len(runs)}
    write_json(output_dir / f"trackeval_grid_summary_{dataset_slug}_{scope}.json", result)
    return result
