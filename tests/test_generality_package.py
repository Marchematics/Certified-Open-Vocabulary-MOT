...
def _patch_phase11_roots(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(phase11, "V2_DIR", tmp_path / "outputs/milestones/reliability_fortress")
    monkeypatch.setattr(phase11, "PHASE11_DIR", tmp_path / "outputs/phase11_release")
    monkeypatch.setattr(phase11, "MILESTONE_DIR", tmp_path / "outputs/milestones/generality_reliability")
    monkeypatch.setattr(phase11, "PACKAGE_PATH", tmp_path / "outputs/packages/generality_reliability.tar.gz")


def _write_release_fixture(root: Path) -> None:
    release = root / "outputs/milestones/reliability_fortress"
    ...
    ).to_csv(release / "audit_labels_2000_human_reviewed.csv", index=False)
    pd.DataFrame(
        [
            {"dataset": "BURST", "alpha1": 0.10, "seed": 0, "released": 1, "UTR": 0.0, "conservative_FTR": 0.0},
        ]
    ).to_csv(release / "table_ovvis_mask_certification.csv", index=False)
    pd.DataFrame(
        [
    ...
    ).to_csv(release / "mask_path_universe.csv", index=False)
    pd.DataFrame(
        [
    ...
    ).to_csv(release / "mask_path_nodes.csv", index=False)
    pd.DataFrame([{"generator": "fixture", "dataset": "OVT-B", "alpha1": 0.10}]).to_csv(
        release / "table_blackbox_generator_certification.csv", index=False
    )
    pd.DataFrame([{"generator": "fixture", "mass_ratio": 1.2}]).to_csv(
        release / "table_prop5_three_generator.csv", index=False
    )
    (release / "second_rater_kappa_report.md").write_text("# fixture\n", encoding="utf-8")
...
def test_ovvis_mask_conflict_thresholds(tmp_path: Path, monkeypatch) -> None:
    ...
    nodes = pd.read_csv(tmp_path / "outputs/milestones/reliability_fortress/mask_path_nodes.csv")
    assert _count_mask_conflicts(nodes, {"a", "b", "c"}, threshold=0.5) == 1


def test_freeze_generality_package_is_public_safe(tmp_path: Path, monkeypatch) -> None:
    _patch_phase11_roots(tmp_path, monkeypatch)
    _write_release_fixture(tmp_path)
    _write_candidate_sources(tmp_path)

    summary = run_phase11_freeze_release()

    milestone = tmp_path / "outputs/milestones/generality_reliability"
    package = tmp_path / "outputs/packages/generality_reliability.tar.gz"
