...
def cross_validated_predictions(data: pd.DataFrame, feature_set: str, feature_names: list[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def summarize_models(metrics: pd.DataFrame, predictions: pd.DataFrame) -> pd.DataFrame:
    ...
    margin_auc = float(out.loc[out["feature_set"].eq("candidate_margin_only"), "mean_roc_auc"].iloc[0])
    system_auc = float(out.loc[out["feature_set"].eq("chemical_system_exploration_only"), "mean_roc_auc"].iloc[0])
    combined_auc = float(out.loc[out["feature_set"].eq("candidate_plus_system"), "mean_roc_auc"].iloc[0])
    out["delta_auc_vs_candidate_margin"] = out["mean_roc_auc"] - margin_auc
    out["system_beats_candidate_margin"] = system_auc >= margin_auc + 0.03
    out["combined_beats_candidate_margin"] = combined_auc >= margin_auc + 0.03
...
def assert_no_t1_feature_leakage() -> None:
    feature_blob = ";".join(sum((v for v in FEATURE_SETS.values()), []))
...
def main() -> None:
    ...
    for feature_set, feature_names in FEATURE_SETS.items():
        pred, metrics = cross_validated_predictions(data, feature_set, feature_names)
