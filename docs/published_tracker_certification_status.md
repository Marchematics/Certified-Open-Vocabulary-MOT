...
```bash
bash ./scripts/phase8_download_published_tracker_weights.sh
```
...
TRACKER=ovtrack DATASET=tao GPUS=4 CUDA_VISIBLE_DEVICES=0,1,2,3 \
  bash ./scripts/phase8_run_ovtrack_family_inference.sh
...
DATASET=tao GPUS=1 CUDA_VISIBLE_DEVICES=0 \
  bash ./scripts/phase8_run_ovtr_inference.sh
```
...
python -m parc_track.cli phase8 published-trackers scaffold \
  --output-dir ./outputs/phase8_published_trackers
```
...
python -m parc_track.cli phase8 published-trackers convert \
  ...
  --out-dir ./outputs/phase8_published_trackers/ovtrack/ovtb
```
...
python -m parc_track.cli phase8 published-trackers matrix \
  --config ./configs/phase8_published_ovtrack_ovtb_matrix.yaml
```
